...
public class KafkaSourceDataTask extends BaseTask {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // parameterTool, KafkaSourceDataTask.class.getSimpleName()
        StreamExecutionEnvironment env = FlinkUtil.getEnv();

        //TODO 6）将kafka消费者对象添加到环境中
        DataStream<String> dataStreamSource = FlinkUtil.createKafkaStream(parameterTool, SimpleStringSchema.class);
        //打印输出测试
        ...
        SingleOutputStreamOperator<ItcastDataObj> errorDataStream = itcastDataObjStream.filter(
                itcastDataObj -> !StringUtils.isEmpty(itcastDataObj.getErrorData()));      // 有空值判断为异常数据
        ...
        // TODO 12)将数据落入到HBase
        // 加入自己写如hbase的sink类 srcDataStream.addSink()
        srcDataStream.addSink(new SrcDataToHBaseSink("itcast_src"));
        //TODO 13）启动作业，运行任务
        env.execute();
    }
}
