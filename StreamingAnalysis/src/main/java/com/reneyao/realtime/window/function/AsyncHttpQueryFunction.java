...
import com.reneyao.realtime.utils.GaoDeMapUtils;
...
public class AsyncHttpQueryFunction extends RichAsyncFunction<OnlineDataObj, OnlineDataObj> {
     ...
     */
    @Override
    public void asyncInvoke(OnlineDataObj onlineDataObj, ResultFuture<OnlineDataObj> resultFuture) throws Exception {
        // try 处理异常的json解析
        try {
            // 1、解析JSON，拿到经纬度信息
            double longitude = onlineDataObj.getLng();
            double latitude =onlineDataObj.getLat();
            ...
            CompletableFuture<OnlineDataObj> orderBeanAndPCD =
                    CompletableFuture.supplyAsync(new Supplier<OnlineDataObj>() {

                        @Override
                        public OnlineDataObj get() {
                            ...
                            try {
                                ...
                                if (response.getStatusLine().getStatusCode() == 200) {
                                    ...
                                    if (regeocodeObject != null && !regeocodeObject.isEmpty()) {
                                        ...
                                        vehicleLocationInfo.setProvince(province);
                                        vehicleLocationInfo.setCity(city);
                                        vehicleLocationInfo.setCounty(district);
                                        vehicleLocationInfo.setLongitude(longitude);
                                        vehicleLocationInfo.setLatitude(latitude);
                                        String geoHash = GeoHashUtil.encode(latitude, longitude);
                                        vehicleLocationInfo.setGeohash(geoHash);
                                        RedisUtil.set(geoHash.getBytes(), JSON.toJSONString(vehicleLocationInfo).getBytes());
                                    }
                                }
                                onlineDataObj.setProvince(province);
                                onlineDataObj.setCity(city);
                                onlineDataObj.setCounty(district);
            ...
            orderBeanAndPCD.thenAccept(new Consumer<OnlineDataObj>() {
                @Override
                public void accept(OnlineDataObj resultBean) {
